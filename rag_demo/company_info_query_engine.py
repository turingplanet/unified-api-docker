...
def run_general_query(query):
    # In your code:
    logger.info(f"Vectorstore file path: {os.path.abspath('./tmp/faiss_vectorstore.pkl')}")
    logger.info(f"Vectorstore file exists: {os.path.exists('./tmp/faiss_vectorstore.pkl')}")
    # Perform a similarity search
    results = vectorstore.similarity_search(query)
    # Extract the content of the top results
    top_results_content = " ".join([result.page_content for result in results[:3]])
    print({"question": query, "context": top_results_content})
